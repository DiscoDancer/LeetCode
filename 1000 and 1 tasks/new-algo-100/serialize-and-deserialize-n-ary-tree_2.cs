public class Codec
{
    ...
    public string serialize(Node root)
    {
        ...
        var nodeToParentIndexTable = new Dictionary<Node, int>();
        ...
        while (queue.Any())
        {
            ...
            for (int i = 0; i < childrenCount; i++)
            {
                ...
                nodeToParentIndexTable[node] = parentIndex;
        ...
        foreach (var (node, parentIndex) in nodeToParentIndexTable)
        {
            sb.Append($"{nodeToIndexTable[node]};{parentIndex}");
            sb.Append('?');
        }
        sb.Remove(sb.Length - 1, 1);

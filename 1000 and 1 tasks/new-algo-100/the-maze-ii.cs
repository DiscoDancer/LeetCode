public class Solution {
    ...
    private void Traverse(int x0, int y0, bool[,] visited, int acc) {
        if (x0 == _destination[0] && y0 == _destination[1]) {
            _result = true;
            _minScore = Math.Min(_minScore, acc);
            return;
        }
        ...
        if (x0 < X-1 && _maze[x0+1][y0] == 0) {
            ...
            if (!visited[maxDown, y0]) {
                ...
                Traverse(maxDown, y0, visited, acc + Math.Abs(x0-maxDown));
        ...
        if (x0 > 0 && _maze[x0-1][y0] == 0) {
            ...
            if (!visited[maxUp, y0]) {
                ...
                Traverse(maxUp, y0, visited, acc + Math.Abs(x0-maxUp));
        ...
        if (y0 > 0 && _maze[x0][y0-1] == 0) {
            ...
            if (!visited[x0, maxLeft]) {
                ...
                Traverse(x0, maxLeft, visited, acc + Math.Abs(maxLeft-y0));
        ...
        if (y0 < Y - 1 && _maze[x0][y0+1] == 0) {
            ...
            if (!visited[x0, maxRight]) {
                ...
                Traverse(x0, maxRight, visited, acc + Math.Abs(maxRight-y0));
    ...
    public int ShortestDistance(int[][] maze, int[] start, int[] destination) {
        ...
        Traverse(start[0], start[1], visited ,0);

        return _result ? _minScore : -1;
    }
}
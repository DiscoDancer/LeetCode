public class Solution
{
    ...
    private void Search(bool[][] visited, Node node, int x, int y)
    {
        ...
        if (x < X - 1 && !visited[x+1][y] && node.Children[_board[x+1][y]-'a']?.Count > 0)
        {
            visited[x + 1][y] = true;
            Search(visited, node.Children[_board[x+1][y]-'a']!, x+1, y);
            visited[x + 1][y] = false;
        }
        if (x > 0 && !visited[x-1][y] && node.Children[_board[x-1][y]-'a']?.Count > 0)
        {
            visited[x - 1][y] = true;
            Search(visited, node.Children[_board[x-1][y]-'a']!, x-1, y);
            visited[x - 1][y] = false;
        ...
        if (y < Y - 1 && !visited[x][y+1] && node.Children[_board[x][y+1]-'a']?.Count > 0)
        {
            visited[x][y+1] = true;
            Search(visited, node.Children[_board[x][y+1]-'a']!, x, y+1);
            visited[x][y+1] = false;
        }
        if (y > 0 && !visited[x][y-1] && node.Children[_board[x][y-1]-'a']?.Count > 0)
        {
            visited[x][y-1] = true;
            Search(visited, node.Children[_board[x][y-1]-'a']!, x, y-1);
            visited[x][y-1] = false;

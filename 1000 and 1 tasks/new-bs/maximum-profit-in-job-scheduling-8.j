...
class Solution {
    ...
    private int bs(int l0, int targetEnd) {

        var l = l0;
        var r = entries.size() - 1;

        while (l <= r) {
            var m = l + (r - l) / 2;
            if (entries.get(m).start >= targetEnd && (m == l0 || entries.get(m - 1).start < targetEnd)) {
                return m;
            } else if (entries.get(m).start < targetEnd) {
                l = m + 1;
            } else {
                r = m - 1;
            }
    ...
    public int jobScheduling(int[] startTime, int[] endTime, int[] profit) {
        ...
        for (int i = entries.size() - 2; i >= 0; i--) {
            ...
            var bsResult = bs(i+1, entries.get(i).end);
            if (bsResult == -1) {
                continue; // не нашли подходящего
            }
            var j = bsResult;

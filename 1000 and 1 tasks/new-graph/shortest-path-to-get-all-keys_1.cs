            ...
            for (int i = 0; i < size; i++)
            {
                ...
                if (x < X - 1 && !visited[x + 1, y] && grid[x + 1][y] != WALL)
                {
                    if (
                        grid[x + 1][y] == PASS ||
                        grid[x + 1][y] == START ||
                        state.Contains(grid[x+1][y]) || //  такой ключ уже есть
                        char.IsUpper(grid[x+1][y]) && state.Contains(char.ToLower(grid[x+1][y]))) // можем открыть
                    {
                        visited[x + 1, y] = true;
                ...
                if (y < Y - 1 && !visited[x, y+1] && grid[x][y+1] != WALL)
                {
                    if (
                        grid[x][y+1] == PASS ||
                        grid[x][y+1] == START ||
                        state.Contains(grid[x][y+1]) || //  такой ключ уже есть
                        char.IsUpper(grid[x][y+1]) && state.Contains(char.ToLower(grid[x][y+1]))) // можем открыть
                    {
                        visited[x, y+1] = true;

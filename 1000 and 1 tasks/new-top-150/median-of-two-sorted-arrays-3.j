...
class Solution {
    ...
    public static int findFirstLesser(int[] arr, int target) {
        int left = 0, right = arr.length - 1;
        ...
        while (left <= right) {
            ...
            if (arr[mid] < target) {
                result = mid; // Store the last found index
                left = mid + 1; // Search in the right half for a possibly larger value still < target
            } else {
                right = mid - 1; // Move left if arr[mid] is >= target
            }
        }
        ...
        while (l <= r) {
            ...
            // м должно быть первым или втором элементом, потом разберемся
                ...
                // если в м лежит правое число, надо найти левое
                    ...
                    var firstLesserThanIndex = findFirstLesser(nums2, nums1[m]);
                    var candidateFromNums2 = Integer.MAX_VALUE;
                    if (firstLesserThanIndex != -1) {
                        candidateFromNums2 = nums2[firstLesserThanIndex];
                    }

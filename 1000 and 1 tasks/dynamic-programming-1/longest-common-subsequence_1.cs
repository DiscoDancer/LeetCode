public class Solution {
    ...
    public int LongestCommonSubsequenceInner(int p1, int p2, int acc) {
        ...
        if (canIncludeCurLetterIn1) {
            var include = LongestCommonSubsequenceInner(p1 + 1, indexInText2 + 1, acc + 1);
            var exclude = LongestCommonSubsequenceInner(p1 + 1, p2, acc);
            return Math.Max(include, exclude);
        }
        return LongestCommonSubsequenceInner(p1 + 1, p2, acc);
    }
